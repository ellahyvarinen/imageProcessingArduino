...
//      myMovieColors[j*numPixelsX + i] = mircam.get(i, j);
      int k = j*numPixelsX + i;
      myMovieColors[k] = mircam.pixels[k];
    }
  }
  calculatesums();
  videoready = true;
  ...
  PImage newimg = createImage(source.width, source.height, RGB);
    for (int x = 0; x < source.width; x++) {
      for (int y = 0; y < source.height; y++) {
        int       loc =                (x)     + y*source.width; // Reversing x to mirror the image
        int mirrorloc = (source.width - x - 1) + y*source.width; // Reversing x to mirror the image
        newimg.pixels[mirrorloc] = source.pixels[loc];
      }
    }
    return newimg;
}

// preprocessing for efficient correlation later
void calculatesums()
{
  for(int j=0;j<numPixelsY;j++)
  for(int i=0;i<numPixelsX;i++)
  {
    ...
    value = colormatch(h,s,b, samplecolor);
...
float colormatch(float h, float s, float b, color sample)
{
//  if(nofiltering) return b;
  float value = 0;
  if(brightness(sample) > 0)
  {
    float bs = brightness(sample);
    float hs = hue(sample);
    float ss = saturation(sample);
    float toler = 50, htoler = 50;
    // look if the pixel color is close enough to the sample color; tolerance of the comparison can be tuned
    if(hs-htoler < 0) { hs += htoler; h += htoler; }
    if(hs+htoler > 255) { hs -= htoler; h -= htoler; }
    
    if(b > bs-toler && h > hs-toler && h < hs+toler && s > ss-toler && s < ss+toler) value = b; 
    else value = 0;
    return value;
  }
  // looking for face color, if no other sample is defined
  boolean inrange = ((h < 20 || h > 230) && (s > 50 && s < 250) && b > 30 && b < 220);
  h -= 230; if(h<0) h+= 256; h -= 23;  // distance
//  if(inrange) value = 10*abs(h);
  if(inrange) value = b;
...
//  if(value < 100) value = 0;  // thresholding
  return value;
}
...
// this implements correlation very efficiently!
{
  ...
  float c = blocksum(x,y, w,h);
  return c;
}

float blocksum(int x, int y, int w, int h)
{
  return sums[x][y] + sums[x+w][y+h] - sums[x+w][y] - sums[x][y+h];
}
